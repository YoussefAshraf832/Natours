...
async function getTour(id) {
  ...
  if (!tours?.data?.data) return tours;
...
function Tour() {
  ...
  useEffect(
    function () {
      if (pathSegments[1] === 'tour') {
        document.title = `Natours | ${tour?.name ? tour?.name : 'All Tours'}`;
      }
    },
    [tour, pathSegments],
  );
  ...
  return (
    <>
      ...
      <section className="section-pictures">
        {tour?.images.map((img) => (
          <TourImages image={img} tour={tour} key={img} />
        ))}
      ...
      <section className="section-map">
        <div id="map">{<Map locations={tour?.locations} />}</div>
        {/* <script>
      ...
      <section className="section-reviews">
        <div className="reviews">
          {tour?.reviews.map((rev) => (
            <ReviewCard review={rev} key={rev.user._id} />
          ))}
